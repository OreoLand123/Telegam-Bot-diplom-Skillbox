...
from format_objects.dataclasses_api import (
    PeopleDataCass,
    SpeciesDataClass,
    VehiclesDataClass,
    PlanetsDataClass,
...
class API:
    ...
    def result_request(
        ...
        if count_value > int(
            request["count"]
        ):  # Проверка на кол-во запросов в случае привышения выдаются все объекты удовлетворяющиее фильтрацию
        ...
        self.object_verification(
            data_json, result, filter_name, arg_param, param_name, value
        )  # Проверка и фильтрация объектов
    ...
    def register_log(
        ...
        """
        user_object_id = (
            session.query(User.id).where(User.tg_id == user_id).first()[0]
        )  # Находим пользователя
        history_data = (
            session.query(Log).join(User).filter(User.tg_id == user_id).all()
        )  # Находим все логи привязанные к нему
        if len(history_data) == 10:  # Проверяем кол-во привязанных к нему логов
            session.delete(
                history_data[0]
            )  # Удаляем первый лог в списке в случае успешной проверки
            session.commit()
        log = Log(
    ...
    def search_obj(
        self, param_name: str, search_input: str, user_id: int, filter_name: str
    ) -> list:
        """
        :param param_name:
        :param search_input:
        ...
        request = requests.get(
            self.url.format(param_name) + f"/?search={search_input}"
        ).json()
