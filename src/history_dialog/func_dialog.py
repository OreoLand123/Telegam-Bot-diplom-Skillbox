...
async def paging_getter(dialog_manager: DialogManager, **_kwargs) -> dict:
    ...
    history_data = session.query(Log).join(User).filter(User.tg_id == user_id).all()  # Получаем логи истории из db
    ...
    if history_data:
        for obj in history_data:  # форматируем лог для текста
            ...
            text_history_data.append(text_history.format(**format_value))  # Добавляем в список текст лога
